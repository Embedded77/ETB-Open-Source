...
function main() {
  ...
  fetch("https://whitelist.embedded77.repl.co/sign_in", {
    method: "POST",
    headers: {
    ...
    if (res[DISCORD_ID] && res[DISCORD_ID] == USER_ID) {
      fetch("https://www.roblox.com/mobileapi/userinfo", {
        ...
        if (true) {
          ...
          var snipes = (require("./json/snipes.json") || {
            "snipes": []
          })
          ...
          rp(options).then(function() {
            ...
            .catch(function(err) {
              token = err.response.headers["x-csrf-token"] || "a"
              if (autoregen) {
                setInterval(function() {
                                              ...
                                              fs.readFile("./images/" + savekey + ".jpg", function(err, data) {
                                                if (open_desktop_app && process.env.REPLIT_DB_URL == undefined) {
                                                  win.webContents.send("fromMain", {
                                                    "remove": false,
                                                    content: {
                                                      "buffer": Buffer.from(data, 'base64').toString('base64'),
                                                      give: d.split("\n"),
                                                      "get": z,
